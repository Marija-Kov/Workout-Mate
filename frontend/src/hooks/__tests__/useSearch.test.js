...
describe("useSearch()", () => {
  ...
  it("should update workout state with search result workouts given that user is authorized", async () => {
    ...
    server.use(
      rest.get(`${process.env.REACT_APP_API}/api/workouts/*`, (req, res, ctx) => {
        return res(
          ...
          ctx.json({
            ...
            allUserWorkoutsMuscleGroups: [...mockWorkouts.map(m => m.muscle_group)],
            total: 2,
            limit: 3,
     ...
     expect(state.workout.workouts.workoutsChunk[0].title).toMatch(mockWorkouts[1].title);
     expect(state.workout.workouts.workoutsChunk[1].title).toMatch(mockWorkouts[0].title);
    const { result } = renderHook(useSearch, { wrapper });
    await act(() => result.current.search(mockWorkouts[1].title.slice(0,1), 0));
