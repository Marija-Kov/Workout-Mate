...
describe("<Chart />", () => {
    it("should render Chart component properly", async () => {
      let muscleGroups = ["leg", "leg", "chest", "forearmAndGrip", "biceps", "triceps", "glute", "triceps"];
        render(
        ...
        const hasLegMuscleGroupWorkouts = await screen.findByLabelText(/25.0% leg/i);
        const hasForearmAndGripMuscleGroupWorkouts = await screen.findByLabelText(/12.5% forearm and grip/i);
        const hasBicepsMuscleGroupWorkouts = await screen.findByLabelText(/12.5% biceps/i);
        const hasTricepsMuscleGroupWorkouts = await screen.findByLabelText(/25.0% triceps/i);
        const hasGluteMuscleGroupWorkouts = await screen.findByLabelText(/12.5% glute/i);
