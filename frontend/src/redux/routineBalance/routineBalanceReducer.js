...
const allMuscleGroups = ["chest", "shoulder", "biceps", "triceps", "leg", "back", "glute", "ab", "calf", "forearmAndGrip"]
...
export const routineBalanceReducer = (state = init, action) => {
    switch(action.type) {
        case SET_ROUTINE_BALANCE: 
         ...
         const balance = {};
         for(let i = 0; i < len; ++i){
            const group = muscleGroups.filter(e => e === allMuscleGroups[i]).length * 100 / total;
