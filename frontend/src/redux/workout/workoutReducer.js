    ...
    switch (action.type){                           
        ...
        case a.UPDATE_ONE_SUCCESS:
          if(action.payload.muscle_group){
           ...
           state.workouts.allUserWorkoutsMuscleGroups.splice(prevMuscleGroupIndex,1);
          }   
          state.workouts.workoutsChunk = state.workouts.workoutsChunk.filter(e => e._id !== action.payload._id );
          const updateMuscleGroups = action.payload.muscle_group ? [...state.workouts.allUserWorkoutsMuscleGroups, action.payload.muscle_group] : state.workouts.allUserWorkoutsMuscleGroups;
            return {
                workouts: {...state.workouts, allUserWorkoutsMuscleGroups: updateMuscleGroups, workoutsChunk:[action.payload, ...state.workouts.workoutsChunk]},
                loading: false,
                updateWorkoutError: null
            }
